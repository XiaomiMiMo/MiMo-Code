...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.devora.desktop.dev",
  beta: "ai.devora.desktop.beta",
  prod: "ai.devora.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.devora.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `devora.settings.dat` is special: it maps to the `devora.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"devora.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // devora.settings.dat → the electron settings store ("devora.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "devora.global.dat").
  const storeName = filename === "devora.settings.dat" ? "devora.settings" : filename
