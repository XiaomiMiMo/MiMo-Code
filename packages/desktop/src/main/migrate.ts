...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "com.xiaomi.mimocode.desktop.dev"
}
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"mimocode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // mimocode.settings.dat → the electron settings store ("mimocode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "mimocode.global.dat").
  const storeName = filename === "mimocode.settings.dat" ? "mimocode.settings" : filename
