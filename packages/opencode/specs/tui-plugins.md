...
- Author package entrypoint is `@mimo-ai/plugin/tui`.
...
- Import types from `@mimo-ai/plugin/tui`.
- `@mimo-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@mimo-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@mimo-ai/plugin/tui`.
