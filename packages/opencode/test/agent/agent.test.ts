...
test("default system prompt has no Claude Code residual and names real dispatch tools", () => {
  ...
  expect(PROMPT_DEFAULT).toContain("exact name from the current tool schema")
  expect(PROMPT_DEFAULT).toContain("display labels")
  expect(PROMPT_DEFAULT).toContain("their schema names may use different casing")
  expect(PROMPT_DEFAULT).not.toContain("Built-in tools use PascalCase")
  expect(PROMPT_DEFAULT).not.toContain("snake_case")
  expect(PROMPT_DEFAULT).toContain("Read, Grep, Glob, Edit, Write, and Bash")
  ...
  // Exact schema-name guidance precedes the display-label examples.
  expect(PROMPT_DEFAULT.indexOf("case-sensitive")).toBeLessThan(PROMPT_DEFAULT.indexOf("display labels"))
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      for (const p of [general?.prompt ?? "", explore?.prompt ?? ""]) {
        ...
        expect(p).toContain("Read, Grep, Glob, Edit, Write, and Bash")
        expect(p).toContain("display labels")
        expect(p).toContain("their schema names may use different casing")
        expect(p).toContain("In GPT/Codex mode, use only the tools actually exposed by that mode")
        expect(p).not.toContain("default harness exposes PascalCase")
