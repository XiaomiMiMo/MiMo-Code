...
it.live("[TP-MCU-R7-21][TP-MCU-R10-20] old generation failure cannot invalidate newer host", () => Effect.gen(function* () {
  let release!: () => void;
  let entered!: () => void;
  let oldLists = 0;
  ...
  const server = yield* Effect.acquireRelease(Effect.sync(() => Bun.serve({
    ...
    async fetch(request) {
      ...
      if (msg.method === "tools/list" && name === "old" && ++oldLists === 2) {
        ...
        return Response.json({jsonrpc: "2.0", id: msg.id, error: {code: -32603, message: "test old generation failed"}});
      }
  ...
  yield* provideTmpdirInstance(() => Effect.gen(function* () {
    ...
    console.log("R014_C8_FAILED_GENERATION", JSON.stringify({tools}));
