...
it.live("[TP-MCU-R7-21][TP-MCU-R10-20] removed host failure must not clear current user connection", () => Effect.gen(function* () {
  let release!: () => void;
  let entered!: () => void;
  let oldLists = 0;
  ...
  yield* provideTmpdirInstance(() => Effect.gen(function* () {
    ...
    console.log("R014_C7_FAILED_REMOVED_HOST", JSON.stringify({tools}));
    expect(tools).toEqual(["example_user"]);
  }));
}));
