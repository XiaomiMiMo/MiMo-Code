...
// Always emit a manifest with a fixed import path so runtime loaders pull overlay
// modules from the dependency graph instead of scanning the filesystem, which
// does not work inside Bun single-file executables. When no overlay is present
// the manifest is empty and the build stays open-source clean.
