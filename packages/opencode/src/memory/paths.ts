...
function normalizePathForMatch(absPath: string): string {
  return absPath.replaceAll("\\", "/")
}

export function parsePath(absPath: string): MemoryLocator | null {
  const m = normalizePathForMatch(absPath).match(/\/memory\/(global|projects|sessions)(?:\/([^/]+))?\/(.+)\.md$/)
  if (!m) return null
...
export function parseCcPath(absPath: string): MemoryLocator | null {
  const m = normalizePathForMatch(absPath).match(CC_PATH_RE)
  if (!m) return null
  const [, slug, keyRaw] = m
  return {
    scope: "cc",
