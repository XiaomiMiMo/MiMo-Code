  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        // Optional local extensions live in src/ext/. The build step generates
        // src/ext/_manifest.ts (empty when no overlay) that statically imports
        // each overlay module, so the bundler pulls them into the compiled
        // binary's dependency graph. We import that manifest with a fixed
        // specifier here; scanning the filesystem does not work inside Bun
        // single-file executables. Dev runs (no build) fall back to a directory
        // scan so overlays still load from source.
