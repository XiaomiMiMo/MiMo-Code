      ...
      async (c) =>
        jsonRequest("SessionRoutes.create", c, function* () {
          ...
          if (conflict.hasConflict) {
            ...
            // Pass worktree directory to session creation (in-process only)
            const svc = yield* SessionShare.Service
            return yield* svc.create({ ...body, directory: info.directory } as Session.CreateInput & { directory: string })
          }
