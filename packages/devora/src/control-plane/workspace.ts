...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    DEVORA_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    DEVORA_WORKSPACE_ID: config.id,
    DEVORA_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.DEVORA_EXPERIMENTAL_WORKSPACES) return
