...
import { DevoraFree } from "../../plugin/devora-free"
import { t } from "../i18n"
import { Instance } from "../../project/instance"
import type { Hooks } from "@devora-ai/plugin"
...
async function devoraFreeLogin() {
  const spinner = prompts.spinner()
  spinner.start(t("cli.providers.devora_free.verifying"))
  try {
    const { fingerprint, exp } = await DevoraFree.verify()
    spinner.stop(t("cli.providers.devora_free.ready"))
    const expDate = new Date(exp).toISOString()
    prompts.log.success(t("cli.providers.devora_free.default_set"))
    prompts.log.info(
      [
        `Endpoint:    ${DevoraFree.chatBaseUrl}/chat`,
        `Fingerprint: ${fingerprint.slice(0, 12)}…${fingerprint.slice(-4)}`,
        `Token exp:   ${expDate}`,
      ].join("\n"),
    )
    prompts.log.info(t("cli.providers.devora_free.usage_hint"))
    prompts.outro("Done")
  } catch (err) {
    spinner.stop(t("cli.providers.devora_free.failed"), 1)
    prompts.log.error(err instanceof Error ? err.message : String(err))
    prompts.outro("Done")
  }
}

async function devoraLogin() {
  ...
  const devoraHook = hooks.findLast((h) => h.auth?.provider === "devora")
  if (!devoraHook?.auth) {
    prompts.log.error("Devora auth plugin not found")
    return
  }

  const method = devoraHook.auth.methods[0]
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (remaining > 0) {
      prompts.log.error(t("cli.providers.devora_login.decrypt_retry", { remaining }))
    } else {
      prompts.log.error(t("cli.providers.devora_login.decrypt_exhausted"))
    }
  }
}
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          ...
          const wellknown = (await fetch(`${url}/.well-known/devora`).then((x) => x.json())) as {
        ...
        let provider: string
        if (args.provider === "devora") {
          await devoraLogin()
          return
        } else if (args.provider === "devora" || args.provider === "devora-free") {
          await devoraFreeLogin()
          return
          ...
          const choice = await prompts.select({
            ...
            options: [
              { label: "Devora", value: "devora", hint: t("cli.providers.devora.recommended_hint") },
              { label: "Devora Auto (free)", value: "devora-free", hint: t("cli.providers.devora_free.hint") },
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in devora.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "devora") {
          prompts.log.info("Create an api key at https://devora.ai/auth")
        }
        ...
        if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
          prompts.log.info(
            "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://devora.ai/docs/providers/#cloudflare-ai-gateway",
          )
        }
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    const info = await AppRuntime.runPromise(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        return yield* auth.get("devora")
      }),
    )
    if (!info) {
      prompts.log.error("Not logged in. Run `devora auth login` to log in.")
      return
    }
    if (info.type === "api" && info.metadata) {
      prompts.log.info(`Provider: Devora`)
