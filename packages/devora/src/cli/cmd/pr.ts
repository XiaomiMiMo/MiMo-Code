...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run devora",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for devora session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found devora session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["devora", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting devora...")
        UI.println()

        const devoraArgs = sessionId ? ["-s", sessionId] : []
        const devoraProcess = Process.spawn(["devora", ...devoraArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await devoraProcess.exited
        if (code !== 0) throw new Error(`devora exited with code ${code}`)
      },
    })
  },
})
