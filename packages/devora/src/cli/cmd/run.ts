...
import { createDevoraClient, type DevoraClient, type ToolPart } from "@devora-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run devora with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running devora server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DEVORA_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: DevoraClient) {
    ...
    async function share(sdk: DevoraClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.DEVORA_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: DevoraClient) {
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.DEVORA_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.DEVORA_SERVER_USERNAME ?? "devora"
        const auth = `Basic ${Buffer.from(`${username}:${password}`).toString("base64")}`
        return { Authorization: auth }
      })()
      const sdk = createDevoraClient({ baseUrl: args.attach, directory, headers })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createDevoraClient({ baseUrl: "http://devora.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
