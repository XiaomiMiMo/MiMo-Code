    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: devora mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in devora.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .devora/ subdirectory too)
  const candidates = [path.join(baseDir, "devora.json"), path.join(baseDir, "devora.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".devora", "devora.json"), path.join(baseDir, ".devora", "devora.jsonc"))
  }
  ...
  // Default to devora.json if none exist
  return candidates[0]
}
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "devora-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
