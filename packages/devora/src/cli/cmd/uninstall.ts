...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall devora and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Devora")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @devora-ai/cli",
      pnpm: "pnpm uninstall -g @devora-ai/cli",
      bun: "bun remove -g @devora-ai/cli",
      // TODO(devora): uncomment when published to these channels
      // brew: "brew uninstall devora",
      // choco: "choco uninstall devora",
      // scoop: "scoop uninstall devora",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Devora!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# devora") || content.includes(".devora/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".devora/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".devora/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".devora"))
    ) {
      continue
    }
