...
const TUI_SCHEMA_URL = "https://devora.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from devora.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const devora = await devoraFiles(input)
  for (const file of devora) {
...
async function devoraFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "devora"),
    ...(await Filesystem.findUp(["devora.json", "devora.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "devora"))
  }
  if (Flag.DEVORA_CONFIG) files.push(Flag.DEVORA_CONFIG)
