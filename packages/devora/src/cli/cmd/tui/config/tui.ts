...
export class Service extends Context.Service<Service, Interface>()("@devora/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.devora`
  // folders between cwd and home, and DEVORA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DEVORA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DEVORA_TUI_CONFIG override, if set.
  if (Flag.DEVORA_TUI_CONFIG) {
    const configFile = Flag.DEVORA_TUI_CONFIG
  ...
  // 4. `.devora` directories (and DEVORA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".devora") || dir === Flag.DEVORA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".devora") && dir !== Flag.DEVORA_CONFIG_DIR) continue
...
async function load(text: string, configFilepath: string): Promise<Info> {
  return ConfigVariable.substitute({ text, type: "path", path: configFilepath, missing: "empty" })
    ...
    .then((data) => {
      ...
      // (mirroring the old devora.json shape) still get their settings applied.
