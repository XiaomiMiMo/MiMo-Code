...
export const dict = {
  ...
  "tui.tips.share":
    "Выполните {highlight}/share{/highlight}, чтобы получить публичную ссылку на диалог на devora.ai",
  ...
  "tui.tips.config_files":
    "Создайте {highlight}devora.json{/highlight} для серверных настроек и {highlight}tui.json{/highlight} для настроек TUI",
  "tui.tips.global_config":
    "Поместите настройки TUI в {highlight}~/.config/devora/tui.json{/highlight} как глобальные",
  "tui.tips.schema":
  ...
  "tui.tips.mcp_oauth":
    "Devora автоматически обрабатывает OAuth для удалённых MCP-серверов с авторизацией",
  "tui.tips.custom_command":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.devora/command/{/highlight} для повторно используемых пользовательских команд",
  ...
  "tui.tips.custom_agent":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.devora/agent/{/highlight} для специализированных AI-персон",
  ...
  "tui.tips.formatter":
    "Devora автоматически форматирует файлы с помощью prettier, gofmt, ruff и других инструментов",
  ...
  "tui.tips.lsp": "Devora использует LSP-серверы для интеллектуального анализа кода",
  "tui.tips.custom_tool":
    "Создавайте файлы {highlight}.ts{/highlight} в {highlight}.devora/tools/{/highlight}, чтобы определить новые LLM-инструменты",
  "tui.tips.tool_scripts":
    "Определения инструментов могут вызывать скрипты на Python, Go и других языках",
  "tui.tips.plugins":
    "Добавляйте файлы {highlight}.ts{/highlight} в {highlight}.devora/plugin/{/highlight} для подключения хуков событий",
  ...
  "tui.tips.plugin_protect":
    "Создайте плагин, который запретит Devora читать чувствительные файлы",
  "tui.tips.run":
    "Используйте {highlight}devora run{/highlight} для неинтерактивных скриптов",
  "tui.tips.continue":
    "Используйте {highlight}devora --continue{/highlight}, чтобы продолжить последнюю сессию",
  "tui.tips.attach_cli":
    "Используйте {highlight}devora run -f file.ts{/highlight}, чтобы прикрепить файлы из командной строки",
  "tui.tips.format_json":
    "Используйте {highlight}--format json{/highlight} для машинно-читаемого вывода в скриптах",
  "tui.tips.serve":
    "Запустите {highlight}devora serve{/highlight} для headless-доступа к API Devora",
  "tui.tips.attach_server":
    "Используйте {highlight}devora run --attach{/highlight}, чтобы подключиться к запущенному серверу",
  "tui.tips.upgrade":
    "Выполните {highlight}devora upgrade{/highlight}, чтобы обновиться до последней версии",
  "tui.tips.auth_list":
    "Выполните {highlight}devora auth list{/highlight}, чтобы увидеть всех настроенных провайдеров",
  "tui.tips.agent_create":
    "Выполните {highlight}devora agent create{/highlight}, чтобы создать агента в пошаговом режиме",
  "tui.tips.github_trigger":
    "Используйте {highlight}/devora{/highlight} в issue/PR на GitHub, чтобы запускать AI-действия",
  "tui.tips.github_install":
    "Выполните {highlight}devora github install{/highlight}, чтобы настроить GitHub-workflow",
  "tui.tips.github_oc":
  ...
  "tui.tips.theme_files":
    "Создавайте JSON-файлы тем в каталоге {highlight}.devora/themes/{/highlight}",
  ...
  "tui.tips.debug_config":
    "Выполните {highlight}devora debug config{/highlight}, чтобы диагностировать проблемы конфигурации",
  "tui.tips.print_logs":
  ...
  "tui.tips.zen":
    "Используйте {highlight}/connect{/highlight} с Devora для подобранных и протестированных моделей",
  "tui.tips.agents_md":
  ...
  "tui.command.devora.status.title": "Посмотреть статус",
  ...
  "tui.voice.error.no_auth": "Сначала войдите в аккаунт Devora",
  ...
  "tui.dialog.login.devora_free.desc": "Анонимный бесплатный канал — вход не требуется",
  "tui.dialog.login.devora_free.success": "Devora Auto (free) готов — модель по умолчанию: devora/devora-auto",
  "tui.dialog.login.devora_free.unavailable": "Провайдер Devora Auto (free) не загружен",

  // CLI: providers command (auth login)
  "cli.providers.select": "Выберите провайдера",
  "cli.providers.other": "Другой провайдер",
  "cli.providers.devora.recommended_hint": "рекомендуется",
  "cli.providers.devora_free.hint": "Анонимный бесплатный канал / devora-auto",
  "cli.providers.devora_free.verifying": "Проверка канала Devora Auto (free)...",
  "cli.providers.devora_free.ready": "Канал Devora Auto (free) готов",
  "cli.providers.devora_free.failed": "Не удалось проверить Devora Auto (free)",
  "cli.providers.devora_free.default_set": "Модель по умолчанию: devora/devora-auto (контекст 1M, бесплатно)",
  "cli.providers.devora_free.usage_hint":
    "Вход не требуется — просто запустите devora. Для платных/премиум-моделей выберите вход через браузер Devora.",
  "cli.providers.devora_login.decrypt_retry": "Ошибка расшифровки, повторите попытку (осталось попыток: {remaining})",
  "cli.providers.devora_login.decrypt_exhausted": "Ошибка расшифровки, превышено максимальное число попыток",
