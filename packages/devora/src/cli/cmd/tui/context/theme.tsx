...
import devora from "./theme/devora.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@devora-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "devora")
        draft.active = typeof active === "string" ? active : "devora"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.devora, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const devoraOrange = RGBA.fromInts(255, 103, 0)

  return {
    theme: {
      // Primary colors using Devora Orange
      primary: devoraOrange,
      secondary: devoraOrange,
      accent: devoraOrange,

      // Status colors using ANSI
      error: ansiColors.red,
      warning: ansiColors.yellow,
      success: devoraOrange,
      info: devoraOrange,
