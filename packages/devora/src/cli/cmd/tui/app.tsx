...
import { DialogDevoraLogin } from "@tui/component/dialog-devora-login"
...
function rendererConfig(_config: TuiConfig.Info, plainTerminal: boolean): CliRendererConfig {
  const mouseEnabled = !plainTerminal && !Flag.DEVORA_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.DEVORA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.DEVORA_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Devora")
      return
    }
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "workflow.list",
      category: "session",
      enabled: Flag.DEVORA_EXPERIMENTAL_WORKFLOW_TOOL,
      slash: {
        name: "workflows",
      },
      onSelect: () => {
    ...
    },
    {
      ...
      onSelect: () => {
        dialog.replace(() => <DialogDevoraLogin />)
      },
      category: "provider",
    },
    {
    ...
    },
    {
      ...
      onSelect: async () => {
        await sdk.client.auth.remove({ providerID: "devora" })
    ...
    },
    {
      ...
      onSelect: () => {
        open("https://github.com/SheriAkhtamov/Devora").catch(() => {})
        dialog.clear()
      },
      category: "system",
    },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Devora v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        ...
        if (
          Flag.DEVORA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT &&
    ...
    >
      <Show when={Flag.DEVORA_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
