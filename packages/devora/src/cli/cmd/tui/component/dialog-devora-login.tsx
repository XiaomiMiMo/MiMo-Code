...
export function DialogDevoraLogin() {
  ...
  return (
    <DialogSelect
      ...
      options={[
        {
          title: t("tui.dialog.login.devora"),
          value: "devora",
          description: t("tui.dialog.login.devora.desc"),
          onSelect: async () => {
            ...
            dialog.replace(() => (
              <DevoraOAuthFlow url={result.data!.url} instructions={result.data!.instructions} />
            ))
          },
        },
        {
          title: t("tui.dialog.login.devora_free"),
          value: "devora-free",
          description: t("tui.dialog.login.devora_free.desc"),
          onSelect: async () => {
            await sync.bootstrap()
            const devora = sync.data.provider.find((p) => p.id === "devora")
            if (!devora || !("devora-auto" in devora.models)) {
              toast.show({ message: t("tui.dialog.login.devora_free.unavailable"), variant: "error" })
              dialog.clear()
              return
            }
            local.model.set({ providerID: "devora", modelID: "devora-auto" }, { recent: true })
            toast.show({ message: t("tui.dialog.login.devora_free.success"), variant: "info" })
            dialog.clear()
          },
        },
        {
...
function DevoraOAuthFlow(props: { url: string; instructions: string }) {
  const dialog = useDialog()
  const sdk = useSDK()
  const sync = useSync()
  const local = useLocal()
  ...
  async function onLoginSuccess() {
    ...
    await sync.bootstrap()
    const devora = sync.data.provider.find((p) => p.id === "devora")
    const defaultModel = devora && "devora-v2.5-pro" in devora.models ? "devora-v2.5-pro" : devora ? Object.keys(devora.models)[0] : undefined
    if (defaultModel) {
      local.model.set({ providerID: "devora", modelID: defaultModel }, { recent: true })
    }
    dialog.clear()
  }
