...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@devora-ai/sdk/v2"
...
export async function runCustomProviderWizard(opts: {
  ...
  const providerIDRaw = await step(1, 6, "Provider id", "e.g. devorarouter")
  if (providerIDRaw === null) return
  const providerID = providerIDRaw.trim()
  if (!providerID) return

  const nameRaw = await step(2, 6, "Display name", "e.g. Devora Router", providerID)
          ...
          devora: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Devora Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://devora.ai/zen</span> to get a key
              </text>
            </box>
          ),
          "devora-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Devora Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://devora.ai/zen</span> and enable Devora Go
              </text>
            </box>
          ),
