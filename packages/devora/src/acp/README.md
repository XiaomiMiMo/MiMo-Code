...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for devora.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal devora sessions
...
devora acp --cwd /path/to/project
```
...
```bash
DEVORA_ENABLE_QUESTION_TOOL=1 devora acp
```
...
- **Terminal Integration**: Full terminal support via devora's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | devora acp
```
...
ACP sessions map cleanly to devora's internal session model:
