...
import type { AssistantMessage, Event, DevoraClient, SessionMessageResponse, ToolPart } from "@devora-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: DevoraClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `devora auth login` in the terminal",
      name: "Login with devora",
      id: "devora-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Devora stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const devoraProvider = providers.find((p) => p.id === "devora")
  if (devoraProvider) {
    if (devoraProvider.models["big-pickle"]) {
      return { providerID: ProviderID.devora, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(devoraProvider.models))
    if (best) {
      return {
