...
const MANAGED_PLIST_DOMAIN = "ai.devora.managed"

// Keys injected by macOS/MDM into the managed plist that are not Devora config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/devora"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "devora")
    default:
      return "/etc/devora"
  }
}
