...
import { NamedError } from "@devora-ai/shared/util/error"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in devora config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for devora serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://devora.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://devora.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@devora/Config") {}

function globalConfigFile() {
  const candidates = ["devora.jsonc", "devora.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://devora.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "devora.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "devora.jsonc"))),
      )
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DEVORA_CONFIG_CONTENT") return "local"
        ...
        const merge = (source: string, next: Info, kind?: ConfigPlugin.Scope) => {
          ...
          mergeMcpOrigins(source, next, "devora")
        ...
        const mergeClaudeMcp = Effect.fnUntraced(function* (source: string) {
          ...
          for (const [name, server] of Object.entries(data.mcpServers)) {
            ...
            if (existing && result.mcp_origins?.[name]?.type !== "claude") {
              log.info(`skipped Claude Code MCP server "${name}"; native devora MCP with same name already exists.`)
              continue
            }
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/devora` })
            const response = yield* Effect.promise(() =>
              fetch(`${url}/.well-known/devora`, { signal: AbortSignal.timeout(1000) }),
            )
            if (!response.ok) {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://devora.ai/config.json"
            const source = `${url}/.well-known/devora`
        ...
        if (Flag.DEVORA_CONFIG) {
          yield* merge(Flag.DEVORA_CONFIG, yield* loadFile(Flag.DEVORA_CONFIG))
          log.debug("loaded custom config", { path: Flag.DEVORA_CONFIG })
        }

        if (!Flag.DEVORA_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("devora", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.DEVORA_CONFIG_DIR) {
          log.debug("loading config from DEVORA_CONFIG_DIR", { path: Flag.DEVORA_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .devora commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".devora") || dir === Flag.DEVORA_CONFIG_DIR) {
            for (const file of ["devora.json", "devora.jsonc"]) {
              ...
              Effect.tap((exit) =>
                Exit.isFailure(exit)
                  ? Effect.sync(() => {
                      ...
                      if (Flag.DEVORA_STRICT_CONFIG_DEPS) log.warn("background dependency install failed", payload)
          ...
          // Auto-discovered plugins under `.devora/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.DEVORA_CONFIG_CONTENT) {
          const source = "DEVORA_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.DEVORA_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from DEVORA_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["DEVORA_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("DEVORA_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (managed) {
          ...
          result = mergeConfigConcatArrays(result, next)
          mergeMcpOrigins(managed.source, next, "devora")
        }

        if (!Flag.DEVORA_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.DEVORA_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.DEVORA_PERMISSION))
        }
        ...
        if (Flag.DEVORA_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.DEVORA_DISABLE_PRUNE) {
