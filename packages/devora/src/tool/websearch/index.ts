...
import * as DevoraWebsearch from "./devora"
import { Auth } from "@/auth"
import { Provider } from "@/provider"
import DESCRIPTION from "./websearch.txt"

const WEBFETCH_FALLBACK =
  "Web search unavailable. Use `webfetch` with a relevant URL instead, or enable the Web Search plugin at https://platform.devora.com/console/plugin."
    ...
    return {
      ...
      execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const result =
            model?.providerID === "devora"
              ? yield* Effect.catchCause(
                  Effect.gen(function* () {
                    const info = yield* auth.get("devora")
                    if (!info || info.type !== "api") return undefined
                    return yield* DevoraWebsearch.call(
                      http,
                      model.api.url,
                      info.key,
                      params.query,
