...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@devora-ai/plugin"
import z from "zod"
...
export class Service extends Context.Service<Service, Interface>()("@devora/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.DEVORA_CLIENT) || Flag.DEVORA_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.patch,
            tool.changedir,
            ...(Flag.DEVORA_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            tool.plan,
            tool.memory,
            tool.history,
            tool.task,
            ...(Flag.DEVORA_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
      ...
      let filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              input.providerID === ProviderID.devora ||
              input.providerID === "devora" ||
              Flag.DEVORA_ENABLE_EXA
            )
          }
          return input.providerID === ProviderID.devora || Flag.DEVORA_ENABLE_EXA
        }
