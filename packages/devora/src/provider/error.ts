...
// Provider IDs served by the Devora model gateway. Its error bodies carry
...
const DEVORA_GATEWAY_PROVIDERS = new Set(["devora", "devora"])

// Devora gateway error.code values worth relabeling: moderation (421) and
...
function message(providerID: ProviderID, e: APICallError) {
  return iife(() => {
    // Devora gateway: relabel known block codes and surface error.param (the real
    ...
    const gw = DEVORA_GATEWAY_PROVIDERS.has(providerID) ? json(e.responseBody)?.error : undefined
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `devora auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
