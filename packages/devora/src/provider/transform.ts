...
// Devora vision support isn't reflected in models.dev modality data, so the
// generic capability check would strip images before they reach the model.
// devora-auto and devora-v2.5 accept images; devora-v2.5-pro is text-only.
function supportsImageInput(model: Provider.Model): boolean {
  if (model.providerID === "devora" || model.providerID === "devora") {
    const id = model.id.toLowerCase()
    if (id.includes("v2.5-pro")) return false
    if (id === "devora-auto" || id.includes("v2.5")) return true
  }
...
export const OUTPUT_TOKEN_MAX = Flag.DEVORA_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
const DEVORA_OUTPUT_TOKEN_MAX = 128_000
...
function limitImages(msgs: ModelMessage[]): ModelMessage[] {
  const maxImages = Flag.DEVORA_MAX_PROMPT_IMAGES
  const maxSize = Flag.DEVORA_MAX_PROMPT_IMAGE_SIZE
...
export function temperature(model: Provider.Model) {
  ...
  if (id.includes("devora")) return 1.0
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "devora" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("devora")) {
      result["promptCacheKey"] = input.sessionID
...
export function maxOutputTokens(model: Provider.Model): number {
  if (model.providerID === "devora" || model.providerID === "devora" || model.id.toLowerCase().includes("devora")) {
    return DEVORA_OUTPUT_TOKEN_MAX
  }
