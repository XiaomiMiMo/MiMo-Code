...
import { Npm } from "../npm"
import { Hash } from "@devora-ai/shared/util/hash"
...
// Tuned for devora-v2.5-pro on Devora Router whose cold-path TTFT after context
...
// per-provider via devora.json's `chunkTimeout` config for tighter or looser
// bounds.
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    devora: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["devora"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from devora.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://github.com/SheriAkhtamov/Devora/coder/",
            "X-Title": "devora",
            "X-Source": "devora",
          },
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `devora/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `devora/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `devora auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `devora/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "devora",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@devora/Provider") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
                ...
                context: (() => {
                  ...
                  if (!warnedContextDefaults.has(key)) {
                    ...
                    log.warn("limit.context not configured and not found in models.dev", {
                      ...
                      fix: `Set limit.context explicitly in devora.json under provider.${providerID}.models.${modelID}`,
                    })
                  }
        ...
        // load env (skipped in devora-only mode so ANTHROPIC_API_KEY etc. don't auto-light other providers)
        if (!Flag.DEVORA_DISABLE_PROVIDER_ENV) {
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.DEVORA_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const devora = s.providers[ProviderID.make("devora")]
      if (devora?.models[ModelID.make("devora-auto")]) {
        return { providerID: devora.id, modelID: ModelID.make("devora-auto") }
      }
