...
import { createDevoraClient } from "@devora-ai/sdk"
import { Flag } from "../flag/flag"
import { CodexAuthPlugin } from "./codex"
import { DevoraAuthPlugin, AnthropicProxyPlugin } from "./devora"
import { DevoraFreeAuthPlugin } from "./devora-free"
...
export class Service extends Context.Service<Service, Interface>()("@devora/Plugin") {}

// Built-in plugins that are directly imported (not installed from npm)
const INTERNAL_PLUGINS: PluginInstance[] = [
  DevoraFreeAuthPlugin,
  DevoraAuthPlugin,
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createDevoraClient({
          ...
          headers: Flag.DEVORA_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.DEVORA_SERVER_USERNAME ?? "devora"}:${Flag.DEVORA_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.DEVORA_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.DEVORA_PURE && cfg.plugin_origins?.length) {
