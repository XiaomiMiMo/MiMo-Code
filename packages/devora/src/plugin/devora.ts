import type { Hooks, PluginInput } from "@devora-ai/plugin"
...
const log = Log.create({ service: "plugin.devora" })

const PLATFORM_URL = process.env.DEVORA_PLATFORM_URL || "https://platform.devora.com"

function getKeyName(): string {
  const filePath = path.join(Global.Path.data, "devora-key-name")
  try {
    const existing = fs.readFileSync(filePath, "utf-8").trim()
    if (existing) return existing
  } catch {}
  const name = `devora-code-cli-key-${crypto.randomBytes(4).toString("hex")}`
...
export async function DevoraAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      input.provider ??= {}
      input.provider.devora ??= {}
      const devora = input.provider.devora
      devora.name ??= "Devora"
      devora.api ??= "https://api.devora.com/v1"
    },
    auth: {
      provider: "devora",
      ...
      methods: [
        {
          ...
          authorize: async () => {
            ...
            log.info("devora oauth server started", { port })
            ...
            const serverCallbackPromise = new Promise<{ sk?: string; uid: string; url?: string }>((resolve, reject) => {
              ...
              server.on("request", (req, res) => {
                ...
                log.info("devora oauth callback received", { path: url.pathname, query: url.search.substring(0, 100) })

                const u = url.searchParams.get("u")

                if (!u) {
                  log.warn("devora oauth callback missing u param")
                ...
                try {
                  ...
                  log.info("devora oauth decrypt success", { uid: result.uid, url: result.url })
                  res.writeHead(302, { Location: `${PLATFORM_URL}/authorize/callback?status=success` })
                  res.end()
                  clearTimeout(timeout)
                  resolve(result)
                } catch (err) {
                  log.error("devora oauth decrypt failed", { error: err })
    ...
    "chat.headers": async (input, output) => {
      if (input.model.providerID !== "devora") return
      output.headers["X-Devora-Source"] = "devora-cli"
    },
  }
}
