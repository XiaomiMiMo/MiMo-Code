...
const log = Log.create({ service: "plugin.devora-free" })

const DEFAULT_BASE_URL = "https://api.devora.com/"
const BASE_URL = (process.env.DEVORA_FREE_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, "")
...
function getClientFingerprint(): string {
  ...
  const file = path.join(Global.Path.data, "devora-free-client")
  try {
...
async function bootstrap(): Promise<{ jwt: string; exp: number }> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`devora-free bootstrap failed: ${res.status} ${body.slice(0, 200)}`)
  }
  const data = (await res.json()) as { jwt?: string }
  if (!data.jwt) throw new Error("devora-free bootstrap response missing jwt")
...
function buildHeaders(init: any, jwt: string): Headers {
  ...
  headers.set("X-Devora-Source", "devora-cli-free")
  return headers
}
...
export async function DevoraFreeAuthPlugin(_input: PluginInput): Promise<Hooks> {
  return {
    config: async (input) => {
      input.provider ??= {}
      input.provider.devora ??= {
        name: "Devora Auto (free)",
      ...
      for (const id of ["devora", "devora-go"]) {
