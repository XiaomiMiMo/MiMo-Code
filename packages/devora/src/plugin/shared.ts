...
export const DEPRECATED_PLUGIN_PACKAGES = ["devora-openai-codex-auth", "devora-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, devoraVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(devoraVersion) || semver.major(devoraVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.devora
  if (typeof range !== "string") return
  if (!semver.satisfies(devoraVersion, range)) {
    throw new Error(`Plugin requires devora ${range} but running ${devoraVersion}`)
  }
}
