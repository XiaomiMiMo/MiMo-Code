...
import { Glob } from "@devora-ai/shared/util/glob"
import { Log } from "../util"
import { Discovery } from "./discovery"
import { extractComposeBundle } from "./compose/extract"

const log = Log.create({ service: "skill" })
const EXTERNAL_DIRS = [".claude", ".agents", ".codex", ".devora"]
const EXTERNAL_SKILL_PATTERN = "skills/**/SKILL.md"
const DEVORA_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.DEVORA_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.DEVORA_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.DEVORA_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.DEVORA_DISABLE_CODEX_SKILLS) return false
      if (dir === ".devora" && Flag.DEVORA_DISABLE_DEVORA_SKILLS) return false
      return true
    })
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, DEVORA_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@devora/Skill") {}
