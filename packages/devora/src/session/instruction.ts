...
const FILES = [
  "AGENTS.md",
  ...(Flag.DEVORA_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.DEVORA_CONFIG_DIR) {
    files.push(path.join(Flag.DEVORA_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.DEVORA_DISABLE_CLAUDE_CODE_PROMPT) {
...
export class Service extends Context.Service<Service, Interface>()("@devora/Instruction") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.DEVORA_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no DEVORA_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.DEVORA_CONFIG_DIR, Flag.DEVORA_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
