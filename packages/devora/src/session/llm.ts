 ...
 *   (configured per-provider via `chunkTimeout` in devora.json). This
...
export class Service extends Context.Service<Service, Interface>()("@devora/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via devora's tool system
      ...
      return streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("devora")
            ? {
                "x-devora-project": Instance.project.id,
                "x-devora-session": input.sessionID,
                "x-devora-request": input.user.id,
                "x-devora-client": Flag.DEVORA_CLIENT,
              }
            : {
                "x-session-affinity": input.sessionID,
                ...(input.parentSessionID ? { "x-parent-session-id": input.parentSessionID } : {}),
                "User-Agent": `devora/${InstallationVersion}`,
              }),
          ...input.model.headers,
          ...headers,
        },
