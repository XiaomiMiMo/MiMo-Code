...
import { Slug } from "@devora-ai/shared/util/slug"
...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in devora, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's devora-native continuation messages in the same session.
          ...
          // Preserve devora-owned metadata on re-sync: keep any user rename
