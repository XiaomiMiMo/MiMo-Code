...
declare const DEVORA_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const binding = require(
      `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${DEVORA_LIBC || "glibc"}` : ""}`,
    )
...
export class Service extends Context.Service<Service, Interface>()("@devora/FileWatcher") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("FileWatcher.state")(
        function* () {
          if (yield* Flag.DEVORA_EXPERIMENTAL_DISABLE_FILEWATCHER) return
          ...
          if (yield* Flag.DEVORA_EXPERIMENTAL_FILEWATCHER) {
