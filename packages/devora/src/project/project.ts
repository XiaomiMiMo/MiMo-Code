...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".devora-project-id")
  const idFile = nodePath.join(mainGit, "devora-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .devora-project-id
  ...
  const existing = await Bun.file(excludeFile)
    .text()
    .catch(() => "")
  if (!existing.includes(".devora-project-id")) {
...
export class Service extends Context.Service<Service, Interface>()("@devora/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.DEVORA_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      if (Flag.DEVORA_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
