...
const Headers = Schema.Struct({
  ...
  "x-devora-directory": Schema.optional(Schema.String),
})
...
class Authorization extends HttpApiMiddleware.Service<Authorization>()("@devora/ExperimentalHttpApiAuthorization", {
  error: Unauthorized,
  security: {
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.DEVORA_SERVER_PASSWORD) return yield* effect

        const user = Flag.DEVORA_SERVER_USERNAME ?? "devora"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.DEVORA_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-devora-directory"] || process.cwd()
