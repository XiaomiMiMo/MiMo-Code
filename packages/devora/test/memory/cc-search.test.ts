...
describe("memory.search with cc scope", () => {
  it.live("ranks CC and devora hits in one result list", () =>
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(devoraRoot, "global", "m.md"), "distinctivetoken from devora"),
          )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() => fs.rm(devoraRoot, { recursive: true, force: true }))
          yield* Effect.promise(() => fs.mkdir(path.join(devoraRoot, "global"), { recursive: true }))
          yield* Effect.promise(() => fs.writeFile(path.join(devoraRoot, "global", "m.md"), "tokenz"))
