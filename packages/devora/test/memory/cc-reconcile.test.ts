  ...
  itLive.live("indexes CC files under scope='cc' with frontmatter type", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const devoraRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(devoraRoot, { recursive: true, force: true }))
...
Body about cache.`
        ...
        yield* Effect.promise(() => reconcileMemory({ devora: devoraRoot, cc: ccBase }))
  ...
  itLive.live("union prune: deleting one CC file leaves devora rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const devoraRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(devoraRoot, { recursive: true, force: true }))
        yield* Effect.promise(() => fs.mkdir(path.join(devoraRoot, "global"), { recursive: true }))
        yield* Effect.promise(() => fs.writeFile(path.join(devoraRoot, "global", "m.md"), "devora body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // devora row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ devora: devoraRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ devora: devoraRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
