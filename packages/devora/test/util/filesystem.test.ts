...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "devora.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "devora.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "devora.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "devora.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["devora.json", "devora.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "devora.json"),
        path.join(tmp.path, "devora.jsonc"),
        path.join(project, "devora.json"),
        path.join(project, "devora.jsonc"),
      ])
    })
  })
