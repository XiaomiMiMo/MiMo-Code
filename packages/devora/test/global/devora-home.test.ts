...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "devora-home-test-"))
  return {
    path: dir,
...
describe("DEVORA_HOME end-to-end", () => {
  test("sets all four paths under the profile root", async () => {
    await using tmp = await tmpdir()
    const result = await runWorker({ DEVORA_HOME: tmp.path })
  ...
  test("auto-creates all four subdirs under a nonexistent root", async () => {
    ...
    const result = await runWorker({ DEVORA_HOME: nested })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test("two profiles have fully isolated data (write in A, not visible in B, A survives restart)", async () => {
    ...
    const resultA = await runWorker({ DEVORA_HOME: a.path })
    ...
    const resultB = await runWorker({ DEVORA_HOME: b.path })
    ...
    const resultA2 = await runWorker({ DEVORA_HOME: a.path })
  ...
  test("relative path causes startup failure with clear error", async () => {
    const result = await runWorker({ DEVORA_HOME: "./relative-path" })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.stderr).toMatch(/DEVORA_HOME must be an absolute path/)
  })
  ...
  test("empty string falls through to XDG behavior", async () => {
    ...
    expect(result.ok).toBe(true)
    if (!result.ok) return
    // Paths should reflect XDG layout (ends with "/devora"), not DEVORA_HOME layout
    expect(result.paths.config).toBe(path.join(tmp.path, "config", "devora"))
    expect(result.paths.data).toBe(path.join(tmp.path, "data", "devora"))
  })
})
