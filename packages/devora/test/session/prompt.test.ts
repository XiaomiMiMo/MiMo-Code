  ...
  test("does not loop empty assistant turns for a simple reply", async () => {
    ...
    const server = Bun.serve({
      ...
      fetch(req) {
        ...
        calls++
        return new Response(chat("packages/devora/src/session/processor.ts"), {
          status: 200,
          ...
          run(
            Effect.gen(function* () {
              ...
              const other = yield* prompt.prompt({
                sessionID: session.id,
                agent: "build",
                model: { providerID: ProviderID.make("devora"), modelID: ModelID.make("kimi-k2.5-free") },
                noReply: true,
...
// F37: subagent context isolation. Devora's spawnSubagent shares
