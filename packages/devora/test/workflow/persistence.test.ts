...
describe("journalKey", () => {
  ...
  test("opts key order does not change the hash", () => {
    const a = journalKey("p", { agentType: "g", model: { providerID: "devora", modelID: "m" } }, 0)
    const b = journalKey("p", { model: { modelID: "m", providerID: "devora" }, agentType: "g" }, 0)
    expect(a).toBe(b)
  })
  ...
  test("a string model ref hashes by its value, not its resolution", () => {
    ...
    expect(journalKey("p", { agentType: "g", model: "devora/devora-v2.5-pro" }, 0)).not.toBe(lite)
