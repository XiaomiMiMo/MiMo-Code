...
//
// Unlike the mock-LLM suite, this drives WorkflowRuntime against the REAL devora
...
// the live router + a real key in ~/.config/devora/devora.json). Run with:
...
// The real devora provider, read verbatim from the user's config (key included).
function realConfig() {
  const home = path.join(os.homedir(), ".config", "devora", "devora.json")
  const full = JSON.parse(readFileSync(home, "utf8"))
  const devora = full.provider?.devora
  if (!devora?.options?.apiKey) throw new Error("no devora provider/key in " + home)
  return { provider: { devora } }
}

const realRef = { providerID: ProviderID.make("devora"), modelID: ModelID.make("devora-v2.5-pro") }
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .devora/workflows.
            const wfDir = path.join(dir, ".devora", "workflows")
