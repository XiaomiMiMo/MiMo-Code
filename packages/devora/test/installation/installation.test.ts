...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @devora/cli-ai is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@devora/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @devora/cli-ai is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@devora/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @devora/cli-ai is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@devora/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @devora/cli-ai", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from devora npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain("pkgs.d.devora.net")
          expect(req.url).toContain(encodeURIComponent("@devora/cli-ai"))
    ...
    test("reads version from devora npm registry for pnpm method", async () => {
    ...
    test("reads version from devora npm registry for bun method", async () => {
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package and registry", async () => {
      ...
      expect(capturedArgs).toContain("@devora/cli-ai@2.0.0")
      expect(capturedArgs.find((a) => a.startsWith("--registry="))).toContain("pkgs.d.devora.net")
    })
