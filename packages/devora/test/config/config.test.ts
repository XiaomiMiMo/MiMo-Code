...
import { EffectFlock } from "@devora-ai/shared/util/effect-flock"
...
const managedConfigDir = process.env.DEVORA_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "devora.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "devora.json") {
...
test("loads Claude Code MCP servers from home and project config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.mcp_origins?.context7?.type).toBe("devora")
...
test("ignores legacy tui keys in devora config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://devora.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "devora.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["DEVORA_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { devora: { options: { apiKey: "{env:DEVORA_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["devora"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["DEVORA_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["DEVORA_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "devora.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .devora directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const devoraDir = path.join(dir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })
      const agentDir = path.join(devoraDir, "agent")
...
test("loads agents from .devora/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const devoraDir = path.join(dir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })

      const agentsDir = path.join(devoraDir, "agents")
...
test("loads commands from .devora/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const devoraDir = path.join(dir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })

      const commandDir = path.join(devoraDir, "command")
...
test("loads commands from .devora/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const devoraDir = path.join(dir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })

      const commandsDir = path.join(devoraDir, "commands")
...
test("devora command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".devora", "command", "dup.md"),
        `---
description: devora version
---
from devora`,
      )
    },
...
test("does not try to install dependencies in read-only DEVORA_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.DEVORA_CONFIG_DIR
  process.env.DEVORA_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.DEVORA_CONFIG_DIR
    else process.env.DEVORA_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable DEVORA_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "devora.json"),
        JSON.stringify({ $schema: "https://devora.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .devora config
      const projectDir = path.join(dir, "project")
      const devoraDir = path.join(projectDir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })
      ...
      // Local .devora config with different plugins
      await Filesystem.write(
        path.join(devoraDir, "devora.json"),
        JSON.stringify({
          $schema: "https://devora.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .devora config with some overlapping plugins
      await Filesystem.write(
        path.join(devoraDir, "devora.json"),
        JSON.stringify({
          $schema: "https://devora.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".devora")
...
// Note: preload.ts sets DEVORA_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .devora config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .devora directory config enables it
      const devoraDir = path.join(dir, ".devora")
      await fs.mkdir(devoraDir, { recursive: true })
      await Filesystem.write(
        path.join(devoraDir, "devora.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/devora")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/devora")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "devora.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-devora@2.4.3", file)).toBe("oh-my-devora@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-devora@2.4.3", "file:///project/.devora/plugin/oh-my-devora.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.devora/plugin/demo.ts", "file:///project/.devora/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.devora/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const devoraDir = path.join(projectDir, ".devora")
        const pluginDir = path.join(devoraDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("DEVORA_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["DEVORA_DISABLE_PROJECT_CONFIG"]
    process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["DEVORA_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .devora/ directories when flag is set", async () => {
    const originalEnv = process.env["DEVORA_DISABLE_PROJECT_CONFIG"]
    process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .devora directory with a command
          const devoraDir = path.join(dir, ".devora", "command")
          await fs.mkdir(devoraDir, { recursive: true })
          await Filesystem.write(path.join(devoraDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .devora should NOT be in directories list
          const hasProjectDevora = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectDevora).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["DEVORA_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DEVORA_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["DEVORA_CONFIG_DIR"]
      process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["DEVORA_CONFIG_DIR"]
      } else {
        process.env["DEVORA_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("DEVORA_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["DEVORA_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DEVORA_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["DEVORA_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["DEVORA_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from DEVORA_CONFIG_DIR, not project
...
describe("DEVORA_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in DEVORA_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DEVORA_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["DEVORA_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://devora.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["DEVORA_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["DEVORA_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in DEVORA_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DEVORA_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Devora Managed",
          PayloadIdentifier: "ai.devora.managed.test",
          PayloadType: "ai.devora.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://devora.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://devora.ai/config.json")
})
