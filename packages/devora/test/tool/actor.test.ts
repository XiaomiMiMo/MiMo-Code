...
describe("Actor tool subagent_type enum (F36)", () => {
  ...
  it.live("flattened schema keeps operation as the sole root key (devora can't drop the discriminator)", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const tool = yield* ActorTool
        const def = yield* tool.init()
        const fakeModel = {
          providerID: "devora",
          api: { id: "devora-v2.5-pro", npm: "@ai-sdk/openai-compatible" },
          id: "devora-v2.5-pro",
          capabilities: { input: {} },
        } as any
        const flat = transformSchema(fakeModel, z.toJSONSchema(def.parameters)) as any
        // Root must expose ONLY `operation`. A flat bag (the bug) lets devora omit
