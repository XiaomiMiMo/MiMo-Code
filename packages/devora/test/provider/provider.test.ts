...
function devoraProviderPresent(providers: Awaited<ReturnType<typeof list>>): boolean {
  return providers[ProviderID.make("devora")] !== undefined
}
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "devora" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".devora", "plugin")
...
test("devora and devora-go providers are disabled by DevoraFreeAuthPlugin", async () => {
  ...
  // DevoraFreeAuthPlugin always pushes devora/devora-go into disabled_providers,
  // so they should not appear even when the user supplies an apiKey or auth record.
  expect(devoraProviderPresent(providers)).toBe(false)
  expect(providers[ProviderID.make("devora-go")]).toBeUndefined()
  // The replacement free provider should be present.
  expect(providers[ProviderID.make("devora")]).toBeDefined()
  expect(providers[ProviderID.make("devora")].models[ModelID.make("devora-auto")]).toBeDefined()
  expect(providers[ProviderID.make("devora")].models[ModelID.make("devora-auto")].limit.context).toBe(1_000_000)
  expect(providers[ProviderID.make("devora")].models[ModelID.make("devora-auto")].limit.output).toBe(128_000)
})
