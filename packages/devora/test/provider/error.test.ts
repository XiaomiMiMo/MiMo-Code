...
const devora = ProviderID.make("devora")
...
describe("provider error message", () => {
  test("maps Devora 421 moderation block (HTTP 400) to a friendly message with param detail", () => {
  ...
  test("maps Devora 441 risk control to a friendly message", () => {
    const parsed = parseAPICallError({
      providerID: devora,
      error: apiError({
  ...
  test("applies friendly gateway mapping for Devora provider variants too", () => {
    const parsed = parseAPICallError({
      providerID: devora,
      error: apiError({
        message: "Moderation Block",
  ...
  test("leaves other providers' error flow completely untouched (robustness)", () => {
    ...
    // gateway-specific code mapping and param enrichment are scoped to Devora
