...
describe("ProviderTransform.maxOutputTokens", () => {
  ...
  test("uses 128K for devora provider models", () => {
    expect(
      ProviderTransform.maxOutputTokens({
        ...baseModel,
        id: ModelID.make("devora-auto"),
        providerID: ProviderID.make("devora"),
      }),
    ).toBe(128_000)
  })
  ...
  test("uses 128K for devora provider models", () => {
    expect(
      ProviderTransform.maxOutputTokens({
        ...baseModel,
        id: ModelID.make("devora-coder"),
        providerID: ProviderID.make("devora"),
      }),
    ).toBe(128_000)
  })

  test("keeps the default cap for non-devora models", () => {
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, devoraModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.devora?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.devora?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.devora?.itemId).toBe("msg_devora")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.devora?.itemId).toBe("msg_devora_part")
...
describe("ProviderTransform.message - cache control on gateway", () => {
  ...
  test("openai-compatible with claude in model id does NOT trigger caching", () => {
    const model = createModel({
      id: "devorarouter/claude-opus-4-8",
      providerID: "custom",
      api: {
        id: "devorarouter/claude-opus-4-8",
