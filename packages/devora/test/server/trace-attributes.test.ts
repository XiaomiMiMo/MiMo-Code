...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under devora.", () => {
    expect(paramToAttributeKey("name")).toBe("devora.name")
    expect(paramToAttributeKey("slug")).toBe("devora.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["devora.sessionID"]).toBeUndefined()
    expect(attrs["devora.messageID"]).toBeUndefined()
    expect(attrs["devora.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under devora. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["devora.name"]).toBe("exa")
