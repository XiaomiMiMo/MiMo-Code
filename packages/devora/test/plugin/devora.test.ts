...
import { DevoraAuthPlugin } from "../../src/plugin/devora"
import type { PluginInput } from "@devora-ai/plugin"
...
describe("DevoraAuthPlugin", () => {
  describe("config hook", () => {
    test("registers devora provider with correct name", async () => {
      const hooks = await DevoraAuthPlugin(fakeInput)
      const cfg: any = {}
      await hooks.config!(cfg)
      expect(cfg.provider.devora.name).toBe("Devora")
      expect(cfg.provider.devora.api).toBeTruthy()
    })
    ...
    test("registers all expected models", async () => {
      ...
      expect(cfg.provider.devora).toBeDefined()
      expect(cfg.provider.devora.name).toBe("Devora")
    })

    test("does not overwrite existing config", async () => {
      const hooks = await DevoraAuthPlugin(fakeInput)
      const cfg: any = { provider: { devora: { name: "Custom", api: "https://custom.api" } } }
      await hooks.config!(cfg)
      expect(cfg.provider.devora.name).toBe("Custom")
      expect(cfg.provider.devora.api).toBe("https://custom.api")
    })
  })

  describe("auth hook structure", () => {
    test("registers auth for devora provider", async () => {
      const hooks = await DevoraAuthPlugin(fakeInput)
      expect(hooks.auth).toBeDefined()
      expect(hooks.auth!.provider).toBe("devora")
    })
  ...
  describe("authorize", () => {
    test("returns url with pk, redirect_uri, kn, key_name params", async () => {
      ...
      expect(url.searchParams.get("kn")).toBe("devora")
      expect(url.searchParams.get("key_name")).toMatch(/^devora-code-cli-key-/)
  ...
  describe("chat.headers hook", () => {
    test("adds X-Devora-Source header for devora provider", async () => {
      const hooks = await DevoraAuthPlugin(fakeInput)
      const output = { headers: {} as Record<string, string> }
      await hooks["chat.headers"]!({ model: { providerID: "devora" } } as any, output as any)
      expect(output.headers["X-Devora-Source"]).toBe("devora-cli")
    })
    ...
    test("does not add header for other providers", async () => {
      ...
      for (const providerID of providers) {
        ...
        expect(output.headers["X-Devora-Source"]).toBeUndefined()
      }
    })
  })
