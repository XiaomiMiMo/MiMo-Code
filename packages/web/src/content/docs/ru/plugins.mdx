...
- `.mimocode/plugins/` – плагины уровня проекта.
...
1. Глобальная конфигурация (`~/.config/opencode/mimocode.jsonc`)
2. Конфигурация проекта (`mimocode.jsonc`)
3. Глобальный каталог плагинов (`~/.config/opencode/plugins/`)
4. Каталог плагинов проекта (`.mimocode/plugins/`)
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://mimocode.ai/docs/sdk).
