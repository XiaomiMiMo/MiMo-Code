...
2. **الإعدادات العامة** (`~/.config/opencode/mimocode.jsonc`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`MIMOCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`mimocode.jsonc` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.opencode`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`MIMOCODE_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
ضع إعدادات OpenCode العامة في `~/.config/opencode/mimocode.jsonc`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `mimocode.jsonc` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `MIMOCODE_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `MIMOCODE_CONFIG_DIR`.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`mimocode.ai/config.json`**](https://mimocode.ai/config.json).
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/opencode/agents/` أو `.mimocode/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/opencode/commands/` أو `.mimocode/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
ضع ملفات الإضافات في `.mimocode/plugins/` أو `~/.config/opencode/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
