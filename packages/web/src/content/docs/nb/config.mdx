...
2. **Global konfigurasjon** (`~/.config/opencode/mimocode.jsonc`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`MIMOCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`mimocode.jsonc` i prosjekt) - prosjektspesifikke innstillinger
5. **`.opencode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`MIMOCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/opencode/mimocode.jsonc`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `mimocode.jsonc` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `MIMOCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `MIMOCODE_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`mimocode.ai/config.json`**](https://mimocode.ai/config.json).
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.mimocode/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.mimocode/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.mimocode/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
