...
- `.mimocode/plugins/` - Plugins em nível de projeto
...
1. Configuração global (`~/.config/opencode/mimocode.jsonc`)
2. Configuração do projeto (`mimocode.jsonc`)
3. Diretório de plugins global (`~/.config/opencode/plugins/`)
4. Diretório de plugins do projeto (`.mimocode/plugins/`)
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://mimocode.ai/docs/sdk) para detalhes.
