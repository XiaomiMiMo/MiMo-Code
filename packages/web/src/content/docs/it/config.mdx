...
2. **Config globale** (`~/.config/opencode/mimocode.jsonc`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `MIMOCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`mimocode.jsonc` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.opencode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `MIMOCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
Metti la configurazione globale di OpenCode in `~/.config/opencode/mimocode.jsonc`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `mimocode.jsonc` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `MIMOCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `MIMOCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`mimocode.ai/config.json`**](https://mimocode.ai/config.json).

La configurazione TUI usa [**`mimocode.ai/tui.json`**](https://mimocode.ai/tui.json).
...
Usa `MIMOCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.mimocode/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.mimocode/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.mimocode/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
