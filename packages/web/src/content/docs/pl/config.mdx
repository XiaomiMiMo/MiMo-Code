...
2. **Konfiguracja globalna** (`~/.config/opencode/mimocode.jsonc`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `MIMOCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`mimocode.jsonc` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `MIMOCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/opencode/mimocode.jsonc`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `mimocode.jsonc` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `MIMOCODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `MIMOCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`mimocode.ai/config.json`**](https://mimocode.ai/config.json).
...
Użyj `MIMOCODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `mimocode.jsonc` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencode/agents/` lub `.mimocode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.mimocode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.mimocode/plugins/` lub `~/.config/opencode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
