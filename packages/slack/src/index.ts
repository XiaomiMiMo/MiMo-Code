...
import { createDevora, type ToolPart } from "@devora-ai/sdk"
...
console.log("🚀 Starting devora server...")
const devora = await createDevora({
  port: 0,
})
console.log("✅ Devora server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await devora.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new devora session...")
    const { client, server } = devora
    ...
    console.log("✅ Created devora session:", createResult.data.id)
  ...
  console.log("📝 Sending to devora:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Devora response:", JSON.stringify(result, null, 2))
