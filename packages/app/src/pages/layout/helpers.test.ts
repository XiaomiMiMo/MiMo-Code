...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("devora://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("devora://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("devora://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("devora://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("devora://open-project")).toBeUndefined()
    expect(parseDeepLink("devora://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "devora://open-project?directory=/a",
      "devora://other?directory=/b",
      "devora://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("devora://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("devora://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("devora://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("devora://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "devora://new-session?directory=/a",
      "devora://open-project?directory=/b",
      "devora://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DEVORA__: {
        deepLinks: ["devora://open-project?directory=/a"],
      },
    } as unknown as Window & { __DEVORA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["devora://open-project?directory=/a"])
