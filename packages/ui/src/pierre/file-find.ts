...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("devora-find")
  api.delete("devora-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("devora-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("devora-find", new Highlight(...rest))
    return true
  }
