...
- An unfinished compaction boundary at the history tail is pending internal work, not a completed history projection. Claimed prompts remain pending until that compaction either commits its summary projection or rolls back; they must not be moved past the boundary and bypass its processing. Successful compaction does not count as first delivery of those prompts; their next ordinary model step records delivery. The real queue is explicitly mounted in `test/session/auto-overflow-writer-first.test.ts`, which covers empty-summary rollback and successful fallback projection without depending on another test's service graph.
