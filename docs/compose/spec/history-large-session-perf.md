...
每个数据库只执行一次本版本的索引升级，打开新目录不会重复触发。新空库直接记为完成；已有索引先清理正文中的媒体编码，保留原来的索引类型，再补齐缺失记录。即使旧版本已经清空索引，或只恢复了一部分，也能从原始消息恢复。原始消息不变。

数据库记录升级版本、处理阶段、已处理位置和本次处理范围。每批索引写入与进度一起提交，批次之间让出执行时间。失败时整批回滚，下次打开数据库从上次成功的位置继续；完成后不再扫描。恢复缺失记录采用默认索引类型，不取决于先打开哪个目录。

新消息和 Claude、Codex、OpenCode、JSON 导入的数据直接维护索引，不需要再扫历史。导入写入与索引写入在同一个事务中完成；删除消息内容时也删除对应索引，避免搜到旧内容。
