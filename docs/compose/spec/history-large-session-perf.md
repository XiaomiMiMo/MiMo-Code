...
目录初始化只请求数据库级的启动扫描。同一进程、同一数据库连接和同一启用索引类型集合共享扫描：并发请求不重复执行，成功完成后后续目录不再扫描。不同类型集合串行执行，空集合不扫描；任一会话扫描失败或任务中断不记为完成，后续初始化可重试。连接关闭后重新打开允许重新扫描。新消息仍由目录级 writer 增量维护，不依赖再次全库扫描。该约束不承诺跨进程免扫描。

回归测试 `packages/opencode/test/history/backfill.test.ts` 的 `[TP-HISTORY-BOOTSTRAP-01]` 验证重复/并发请求只扫描一次、配置顺序无关、配置变更、空配置和失败重试；已有 writer 与 history 工具测试验证增量写入及检索。
