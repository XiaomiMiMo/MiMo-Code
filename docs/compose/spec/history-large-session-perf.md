...
数据库打开后启动一次后台兼容迁移，与目录初始化及目录配置无关。持久化状态包含版本、阶段、游标及启动时的行号上界；不能用表存在、索引非空或旧完成标记推断迁移成功。已有索引正文原位清洗，保留其 kind，更新触发器维护 FTS5；随后仅恢复缺失索引的历史 part，使用固定默认 kinds，不读取首个目录配置。此前 DELETE 已执行或只恢复一部分的数据库均可完成恢复，原始 part 不变。

每批索引写入与游标推进在同一个事务提交，批次间让出事件循环。崩溃或失败不提交失败批次的游标，重启从最后提交位置继续；同一连接不循环重试。完成后永久跳过此版本，不因重启、打开目录或修改 kinds 再扫描。新消息由 writer 增量维护，Claude/Codex/OpenCode/JSON 导入在写入 part 的事务中同步维护索引，采用默认 kinds，因此迁移上界之后的新数据无需补齐。

删除原始 part（包括导入替换消息的级联删除）时，在同事务删除对应派生索引，避免残留旧搜索结果。

回归映射：`[TP-HISTORY-BOOTSTRAP-01]` 数据库启动与多目录零重复迁移；`[TP-HISTORY-BOOTSTRAP-02]` 真实迁移、部分恢复、批次失败回滚、跨进程断点（`packages/opencode/test/history/backfill.test.ts`）；`[TP-HISTORY-IMPORT-01]` 四类真实导入与搜索结果同步（`packages/opencode/test/history/import.test.ts`、`packages/opencode/test/storage/json-migration.test.ts`）。

索引内容规则升级优先原位转换已有派生正文；只有无法由已有正文转换的版本才从原始数据构建。兼容恢复期间搜索可能暂不完整。回归覆盖部分索引、空库、断点恢复、失败事务回滚、导入后搜索及目录初始化零扫描。
