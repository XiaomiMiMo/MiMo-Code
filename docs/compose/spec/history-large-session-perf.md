...
全文索引的历史扫描仅用于迁移后的单次重建。迁移写入持久化完成状态：已有索引内容或没有历史 part 的库直接完成；有历史 part 且索引被清空的库标为待重建。已有索引不补齐漏项。首次启用索引时后台执行待重建任务，全部会话成功后落库完成；失败或中断保留待重建状态，下一进程启动可重试，同一进程不反复重试。

完成后重启、初始化目录、修改索引类型均不再扫描历史；新消息由 writer 增量维护。完成状态独立于索引行数，因此没有可索引内容的空结果也不会再次扫描。进程内按数据库连接只尝试一次，避免多个目录同时触发同一迁移任务。

回归测试 `packages/opencode/test/history/backfill.test.ts` 的 `[TP-HISTORY-BOOTSTRAP-01]` 验证并发与目录去重、禁用配置、已完成跳过；`[TP-HISTORY-BOOTSTRAP-02]` 验证真实迁移及跨进程重启、空结果、失败重试。已有 writer 与 history 工具测试验证增量写入及检索。
