...
数据库记录升级版本、处理阶段、已处理位置和本次处理范围。每批索引写入与进度一起提交，批次之间让出执行时间。失败时整批回滚，下次打开数据库从上次成功的位置继续；完成后不再扫描。本版本统一索引用户文本、助手文本、思考文本、工具输入与结果、文件名和 MIME 类型，不提供 `history.kinds` 配置；旧配置中的该字段被忽略。已有的仅含工具输入的索引也会一次性更新，后续不因目录或配置变化重扫。
