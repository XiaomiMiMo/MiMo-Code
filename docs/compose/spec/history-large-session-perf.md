...
数据库记录升级版本、处理阶段、已处理位置和本次处理范围。每批索引写入与进度一起提交，批次之间让出执行时间。失败时整批回滚，下次打开数据库从上次成功的位置继续；完成后不再扫描。本版本统一索引用户文本、助手文本、思考文本、工具输入与结果、文件名和 MIME 类型，不提供 `history.kinds` 配置；旧配置中的该字段在读取和全局配置写回时均被忽略，JSONC 注释保留。已有的仅含工具输入的索引也会一次性更新，后续不因目录或配置变化重扫。

搜索类型过滤提供 user_text、assistant_text、reasoning、tool_output、tool_error、file；成功工具的输入与输出同属 tool_output，失败工具的输入与错误同属 tool_error，两种终态均索引附件文件名和 MIME。
