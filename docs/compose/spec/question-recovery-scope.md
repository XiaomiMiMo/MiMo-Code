...
Physical database archiving, new indexes/migrations, background transcript scans, desktop application changes, new question replay UI, and redesigning actor abandonment or cross-process ownership are outside this fix. Large individual-session hydration costs remain a separate concern.
...
- [ ] T3: Verify selected-session recovery and tool cleanup — acceptance: pending/running questions in an idle selected main slice are repaired on fresh prompt, unrelated session/subagent history and busy/retry work are untouched, recovery discovery remains read-only; package typecheck and Node build pass (covers: S2; depends: T2).
