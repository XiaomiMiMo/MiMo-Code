...
To run MiMoCode in the root of the repo itself:
...
During development, `bun dev` is the local equivalent of the built `mimocode` command. Both run the same CLI interface:
...
# Production
mimocode --help          # Show all available commands
mimocode serve           # Start headless API server
mimocode web             # Start server + open web interface
mimocode <directory>     # Start TUI in specific directory
```
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/opencode ./src/index.ts serve --port 4096`,
    then attach TUI with `mimocode attach http://localhost:4096`
...
- `chore(opencode):` maintenance in the core package
